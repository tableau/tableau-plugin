...
catalog (see `tableau-workbook-templating`), run the local
...
If the workbook was instead hand-edited as raw XML (see
`tableau-workbook-authoring`), run
`../../scripts/validate_workbook.py <file>` first instead — the same
principle applies: that is a structural check, not the pre-publish gate.
